...
def get_ner_pipeline():
    """Return (pipeline_or_None, model_name, init_warning_or_None).

    Compatible with backend.services.symptoms expectations.
    """
    model_name = os.getenv("BIOBERT_MODEL_NAME", "d4data/biobert_ner")
    pipe = _lazy_pipeline()
    if pipe is None:
        return None, model_name, "transformers or model not available"
...
__all__ = ["get_ner_pipeline", "format_entities", "detect_medical_entities"]
